...
> **Identidad VIGENTE: neobrutalismo cálido ("álbum de pegatinas").** Este documento describe el
> sistema que YA está implementado en `tokens.css` y en los CSS Modules. Reemplaza a la identidad
> anterior (una guía de campo instrumental, de acentos metálicos sobre azul marino). El resumen de
> adopción de este rediseño está en `docs/redesign-neobrutalismo.md`; los valores de verdad, en
> `tokens.css`. Donde el texto menciona la identidad anterior, lo hace como contexto histórico, nunca
> como lo vigente.

---

## 1. Concepto: "Álbum de pegatinas neobrutalista cálido"

La app no es un test genérico: es un **álbum de pegatinas coleccionables**. Las banderas ya son campos
de color planos con bordes duros — así que el sistema las trata por lo que son: **stickers sobre papel
crema**, montados con **contornos de tinta gruesos**, **sombras duras desplazadas** (offset sin
desenfoque) y **física de "presionar la pegatina"** en cada control.

Punto de vista: las banderas son ruidosas y a todo color — todo el espectro está en ellas. Por eso **el
sistema no compite en color**: fondos crema cálidos, tinta negra para texto y contornos, y un único
color interactivo muy medido (**amarillo**). Las banderas van montadas como **especímenes** sobre
blanco fiel; el cromatismo lo ponen ellas y el sistema solo las troquela y las hace saltar del papel.

Anclas del tema que usamos de verdad (no como decorado): **las banderas como objetos planos de borde
duro** (por eso el tratamiento de sticker: contorno + sombra dura las convierte en pegatinas
coleccionables); la **física táctil** del álbum (presionar, despegar, pegar torcido); y el **mono como
etiqueta de índice** (códigos ISO, progreso `03 / 10`, capital/población — lecturas rotuladas, como el
lomo de una ficha de colección).

### Por qué NO es un default de IA
La skill `frontend-design` nombra tres clusters genéricos de IA; el neobrutalismo aquí es una elección
**deliberada** para este sujeto concreto (banderas = objetos planos de borde duro → álbum de stickers),
y esquiva los tres:
- **No** crema (#F4F1EA) + serif de alto contraste + terracota → nuestro crema (#FAF1E2) se combina con
  **contornos de tinta negra + amarillo**, no con terracota + serif. La display es una **grotesca
  chunky** (Bricolage a peso 800), nunca una serif editorial.
- **No** casi-negro + un acento ácido/bermellón → base **clara** (crema es el light principal) y el
  acento es **amarillo cálido** (#FFD935), explícitamente **no el lima ácido** de un primer intento
  descartado. Rojo y verde quedan **solo** para lo semántico (acierto/error).
- **No** broadsheet (filetes, radio 0, columnas de periódico) → tenemos **radios chunky** (6–20px),
  **sombras duras** desplazadas y **textura de puntos**, no filetes ni columnas.

---

## 2. Paleta (6 nombres + semánticos)

Base crema cálida para que las banderas sean el color. El borde **ES tinta** (mismo negro para
hairlines y contornos: jerarquiza el **grosor**, no el color). Un solo color interactivo (amarillo),
usado con disciplina: CTA, marcador de selección, foco; y el ámbar para lo decorativo.

| Nombre           | Rol                                        | Hex (light) |
|------------------|--------------------------------------------|-------------|
| **Crema**        | Fondo de app (papel cálido)                | `#FAF1E2`   |
| **Papel**        | Superficie: tarjetas, opciones, ledgers    | `#FFFDF8`   |
| **Hundida**      | Superficie hundida / bandas de grupo       | `#F1E6CF`   |
| **Celeste**      | Ficha cultural / hoja de feedback          | `#CDE9EF`   |
| **Tinta**        | Texto primario **Y** todos los contornos   | `#17150F`   |
| **Ámbar**        | Acento decorativo: progreso, ticks, eyebrows, récords | `#F59E0B` |
| **Amarillo**     | Superficie interactiva: CTA y marcador de selección | `#FFD935` |
| **Verde sello**  | Acierto (semántico)                        | `#3E9B4F`   |
| **Rojo señal**   | Error (semántico)                          | `#E23D45`   |

Tinta y contornos (el borde es tinta):
- `--c-ink #17150F` (texto primario y contornos), `--c-ink-2 #4A4438` (secundario), `--c-ink-3 #6E6656`
  (captions, placeholders).
- `--c-border` y `--c-border-strong`, **ambos `#17150F`**: el color no distingue jerarquía, el **grosor
  de trazo** sí (ver §6).

Amarillo/ámbar (el color interactivo del sistema):
- `--c-accent #F59E0B` (**ámbar decorativo**: barras de marca, relleno de progreso, ticks, eyebrows,
  viñetas de facts, récords).
- `--c-accent-strong #FFD935` (**superficie de CTA**: amarillo con texto tinta encima),
  `--c-accent-tint #FFF1BF` (fondos suaves / fila seleccionada), `--c-accent-ink #92400E` (**ámbar
  quemado**, AA para texto ámbar sobre claro: eyebrows, récords), `--c-on-accent #17150F`.

Marcador de selección (fijo en ambos modos):
- `--c-marker #FFD935` / `--c-on-marker #17150F`: **amarillo pleno con tinta negra encima**, para las
  superficies "elegidas" — pestaña activa, thumb del `SegmentedControl`, chip elegido, opción marcada.
  Existe **aparte** de `--c-note` (celeste) porque el celeste, al ser una superficie grande, se **apaga
  en dark** (pasa a `#1E2E33` bajo tinta clara); el amarillo del marcador, en cambio, debe seguir siendo
  amarillo pleno con tinta negra **en ambos modos**, así que necesita su propio par de tokens que no
  cambia con el tema.

Semánticos (rellenos planos con tinta oscura encima, **solo** para acierto/error):
- Verde: `--c-success #3E9B4F`, `--c-success-tint #C4E8C6`, `--c-success-ink #1B5226`.
- Rojo: `--c-error #E23D45`, `--c-error-tint #F9C9C7`, `--c-error-ink #841016`.

Otros:
- `--c-focus: var(--c-ink)` (foco en **tinta gruesa**; el trazo + offset ya lo hace visible).
- `--c-flag-bg #FFFFFF` (interior de la tarjeta de bandera, **blanco fiel** para color exacto).
- `--c-shadow #17150F` (color de las sombras **duras**; en dark pasa a negro puro).

Contraste: Tinta sobre Crema **> 12:1**; `--c-ink-2` / `--c-ink-3` sobre Crema cumplen **AA** en su
tamaño; el amarillo (CTA y marcador) **siempre lleva tinta oscura encima** (`--c-on-accent` /
`--c-on-marker`), nunca texto claro sobre color saturado; el ámbar quemado (`--c-accent-ink`) es AA
sobre claro para eyebrows/récords.

### Dark (secundario, vía `@media (prefers-color-scheme: dark)`)
La tinta se **invierte**: contornos crema (`--c-border #F7F1E3`) sobre **carbón cálido** (`--c-bg
#1B1810`), tarjetas `--c-surface #2A261A`, hundida `--c-surface-2 #14110B`, celeste profundo `--c-note
#1E2E33`. Texto `--c-ink #F7F1E3` / `--c-ink-2 #CBC3AF` / `--c-ink-3 #9A9280`. El **amarillo sigue siendo
el interactivo** (`--c-accent #FBBF24`, `--c-accent-strong #FFD935`, `--c-accent-ink #FCD34D`,
`--c-on-accent #17150F`), y `--c-marker` / `--c-on-marker` **no cambian** (amarillo pleno + tinta
negra). Semánticos aclarados (`--c-success #57C06A`, `--c-error #F26A6F`, con sus tint/ink). Las
**sombras pasan a negro puro** (`--c-shadow #000`) porque la superficie es más clara que el fondo y el
bloque de sombra debe leerse. El **interior de la bandera se mantiene claro** (`--c-flag-bg #EEF2F5`)
para que los colores de las banderas sean fieles. Valores completos en `tokens.css`.

---

## 3. Tipografía

Pareja deliberada, ninguna de las que caerían "por defecto" (nada de Playfair/serif de revista ni Inter
neutro). El sistema no usa serif: la voz es una **grotesca chunky**; el mono aporta el aire de
**etiqueta de índice** en los datos.

- **Display — Bricolage Grotesque** (`@fontsource-variable/bricolage-grotesque`).
  Grotesca contemporánea de terminaciones ligeramente irregulares, que **a peso 800**
  (`--weight-display`) es la voz **"póster/sticker"** del sistema: chunky, segura, memorable. Titulares,
  nombres de país (etiqueta de espécimen), eyebrows. Pesos 600–800; el **800** es la firma. Tracking
  negativo en tamaños grandes (`--tracking-tight`).
...
- **Datos — Spline Sans Mono** (`@fontsource-variable/spline-sans-mono`).
  Mono para las **lecturas rotuladas**: códigos ISO (`FR`), progreso (`03 / 10`), capital/población,
  cifras de récord. Es la "etiqueta de índice" del álbum. Peso 400/500, en tamaños pequeños con
  `--tracking-normal`.
...
| `--text-4xl`  | 48  | hero grande / marcador del resultado |
...
## 4. Elemento signature: el **tratamiento de sticker**

El signature del sistema es el **tratamiento de pegatina**: **contorno de tinta grueso** (`--border-mid`
2px / `--border-bold` 3px) + **sombra dura desplazada** (offset en tinta, sin blur) + **física de
pulsación**. En reposo la pegatina flota sobre el papel; al **hover se despega** (`translate(-1px)` o
`-2px`, la sombra crece un paso); al **active se hunde** contra el papel (`translate(+--press-shift)`,
3px, y la sombra **colapsa a 0** — la pieza recorre exactamente su sombra). En la tarjeta de bandera,
ese contorno + sombra son la marca de **"espécimen coleccionable"** y **resuelven el problema de las
banderas con blanco** (Japón, Nigeria en zonas, etc.): la pegatina se lee como objeto sobre el papel sin
depender de su contenido.

> La antigua barra vertical junto al borde de la tarjeta de bandera (herencia de la identidad anterior)
> **se retiró** de `FlagImage`: con el contorno + la sombra dura ya hay marca de espécimen, y la barra
> parecía un artefacto.

**El lenguaje de SELECCIÓN es la pegatina amarilla** (`--c-marker` #FFD935 con tinta negra encima): lo
elegido se vuelve amarillo pleno (opción marcada, chip elegido, thumb del segmentado, pestaña activa).

**Refuerzos del signature (atmósfera de apoyo, con restricción):**
- **Barra-marcador** (herencia del hoist, ahora en ámbar): una barra vertical de `--hoist-width` (**6px**)
  en **Ámbar** (`--c-accent`) que queda **solo** en el canto de la **hoja de feedback**, la **FactCard**
  y el **aviso de instalación** — un acento de borde que dice "esto es una ficha", no un semáforo.
- **Trama de puntos** (`--graticule`): un `radial-gradient` de puntos finos (`--graticule-line`, tamaño
  `--graticule-size` 22px) como fondo de cabeceras (masthead, cabecera de Explorar). Nunca sobre
  contenido denso ni sobre banderas.
- **Barra de progreso = regla**: pista de superficie con **ticks de tinta tenue**; el relleno es **Ámbar**
  (`--c-accent`). El contador `03 / 10` en mono.
- **Datos en mono**: ISO, capital, población, % de acierto — como etiquetas de índice.

Regla de Chanel ("quita un accesorio"): el signature es el **tratamiento de sticker** (contorno + sombra
dura + pulsación) y el **amarillo interactivo**. La barra-marcador, la trama de puntos, los ticks y el
mono son ecos discretos del mismo mundo; todo lo demás se mantiene callado. Si una pantalla compite con
las banderas, gana la bandera.
...
- Contenedor con `aspect-ratio: 3 / 2`; la bandera es `<img>` con `object-fit: contain` sobre
  **`--c-flag-bg` (#FFFFFF)** (blanco real para color fiel), centrada, con `padding` interior de
  `--space-2` para que el borde de la bandera no toque el marco.
- **Tratamiento de sticker**: `border: var(--border-mid) solid var(--c-border)` (2px de tinta),
  `border-radius: var(--radius-lg)` (14px), `box-shadow: var(--shadow-sm)` (**sombra dura** 3px 3px 0).
  **Este contorno + sombra es obligatorio** para separar banderas claras/blancas del fondo — es el
  signature (§4).
...
Superficie base **Papel** (`--c-surface`), **contorno de tinta** (`--border-mid`), radio `--radius-md`
(10px), min-height 56px, padding `--space-4`, texto `--text-lg`, **sombra dura** `--shadow-sm`. En
`flag-to-name` la etiqueta es el nombre (alineado a la izquierda). En `name-to-flag` el "botón" contiene
una mini tarjeta de bandera (5.1 reducida). Física de pegatina en todos: hover **despega**, active
**hunde**.

| Estado          | Fondo               | Borde                 | Texto/Detalle |
|-----------------|---------------------|-----------------------|---------------|
| **Normal**      | `--c-surface`       | `--c-border` (2px)    | Tinta; `--shadow-sm`; hover `translate(-2px)` + la sombra crece; active `translate(+3px)` + sombra a 0 |
| **Seleccionada**| `--c-marker`        | `--c-border` (2px)    | `--c-on-marker` (tinta sobre amarillo pleno): la pegatina "elegida" mientras se confirma (si el modo lo usa) |
| **Correcta**    | `--c-success-tint`  | `2px --c-success`     | Texto `--c-success-ink`; icono ✓ a la derecha; *stamp* + halo verde (ver Motion) |
...
events: none`. Todas las opciones quedan `aria-disabled` tras responder.

### 5.3 Input del modo "escribir el nombre"
- Campo grande: alto 56px, `--text-lg`, padding `--space-4`, fondo `--c-surface`, contorno de tinta
  `--c-border` (`--border-mid`), radio `--radius-md`. Placeholder en `--c-ink-3`: "Escribe el país…".
- Foco: `outline` de tinta gruesa (`--c-focus`) + `box-shadow: var(--shadow-focus)`.
- Ayuda debajo, en mono `--text-xs` `--c-ink-3`: "No importan tildes ni mayúsculas".
- Botón "Comprobar" (Button primario amarillo) a ancho completo debajo, o icono de envío dentro del
  campo.
- Tras enviar: el campo se bloquea y se pinta el borde en Verde sello (acierto) o Rojo señal
...
- Material distinto del chrome de papel: fondo **Celeste** (`--c-note` #CDE9EF), la pegatina de la ficha
  cultural.
- **Barra-marcador en Ámbar** a la izquierda (`--hoist-width`, 6px, `--c-accent`), **contorno de tinta**
  + **sombra dura** (`--shadow-sm`), radio `--radius-lg`.
...
- Encabezado de estado del feedback: banda superior compacta con ✓ Verde sello "¡Correcto!" o ✕
...
- Grid de 2 columnas en móvil (`gap: --space-3`). Cada tarjeta: mini tarjeta de bandera (5.1) arriba
  (aspect 3/2, contorno + sombra), luego nombre en display `--text-lg` (2 líneas máx,
  `text-wrap: balance`), y una línea mono `--text-2xs` con ISO + continente.
- Tap: toda la tarjeta es enlace a `/explorar/{code}`; `--shadow-sm`, hover/active **despega**
  (`--shadow-md` + `translate(-2px)`); radio `--radius-lg`.

### 5.6 Barra de progreso (regla)
- Pista: alto 8px, fondo **Hundida** (`--c-surface-2`), radio `--radius-pill`, con **ticks** de tinta
  tenue (un `repeating-linear-gradient` sutil) que evocan una regla graduada.
- Relleno: **Ámbar** (`--c-accent`), radio pill, transición `width var(--dur-slow) var(--ease-out)`.
...
- **SegmentedControl** (modo, nº de preguntas): contenedor pill `--c-surface` con **contorno de tinta**
  `--c-border`; el segmento activo lo marca una **pegatina amarilla que se desliza** (thumb `--c-marker`
  con sombra dura), texto del segmento activo `--c-on-marker`, peso 600. Técnica y física completas en
  §26.2. Táctil ≥44px.
- **ContinentPicker**: chips (pills) seleccionables con contorno de tinta; **elegido = pegatina amarilla**
  (`--c-marker` + `--c-on-marker`).
- **Button primario** ("Empezar", "Jugar otra vez"): fondo **amarillo** `--c-accent-strong`, texto
  `--c-on-accent`, **contorno de tinta** `--border-bold` (3px), **sombra dura**, radio `--radius-md`,
  alto 52px, ancho completo en móvil, peso 600, física de pulsación. Secundario: fondo `--c-surface`,
  contorno de tinta, texto Tinta.

### 5.8 Hero de Home (tesis de la página)
Lo más característico del mundo del sujeto abre la página: **una bandera montada como espécimen** (con
su tratamiento de sticker) sobre la **trama de puntos**, con el título en display y el eyebrow en mono
("Guía de campo · 195 países"). No usar el patrón plantilla (número gigante + stats + gradiente). El
héroe es la bandera + el gesto de "empezar a identificar".
...
- **Radios**: `--radius-sm 6`, `--radius-md 10`, `--radius-lg 14`, `--radius-xl 20`,
  ...
  el look broadsheet; esquinas chunky, como pegatinas troqueladas.)
- **Espaciado** (base 4px): escala `--space-1..--space-20` (4, 8, 12, 16, 20, 24, 32, 40, 48, 64,
  80). Ritmo vertical de secciones en `--space-6/--space-8`.
- **Bordes = tinta, jerarquía por grosor**: `--border-thin 1px` (hairlines/separadores internos),
  `--border-mid 2px` (tarjetas y controles), `--border-bold 3px` (CTAs y marco de página).
  `--hoist-width 6px` (barra-marcador lateral).
- **Sombras DURAS** (offset sin blur, color `--c-shadow`, tinta): `--shadow-sm 3px 3px 0`,
  `--shadow-md 5px 5px 0`, `--shadow-lg 9px 9px 0`, `--shadow-thumb 2px 2px 0` (piezas pequeñas:
  thumb, chips), `--shadow-focus 0 0 0 3px rgba(23,21,15,.30)`. `--press-shift 3px` (la pulsación
  recorre exactamente su `--shadow-sm`). En **dark** las sombras son **negro puro**.

---

## 7. Motion (física de sticker + pop; respeta `prefers-reduced-motion`)

Duraciones `--dur-fast 120ms`, `--dur 200ms`, `--dur-slow 320ms`, `--dur-slide 220ms`. Easings
`--ease-out`, `--ease-standard`, `--ease-spring` (asentados), y **`--ease-pop`
`cubic-bezier(0.25,1.65,0.4,1)`** (overshoot cartoon de las apariciones).

- **Física base (todo control es una pegatina)**: hover **LEVANTA** (`translate(-1/-2px)`, la sombra
  crece un paso); active **HUNDE** (`translate(+--press-shift)`, 3px, la sombra colapsa a 0).
- **Entrada de pregunta**: la pregunta hace **pop** (`popIn`, `--ease-pop`).
- **Hoja de feedback**: **sube con rebote** (translate desde abajo con `--ease-pop`).
- **Sello de resultado**: el ✓/✕ va en un **cuadradito girado −4°** que **cae estampándose**.
- **Acierto**: la opción/tarjeta hace **stamp** (escala con leve rotación) al pintarse de verde, +
  **halo verde** que florece y se apaga (`correctFlash`, ver §23.2).
- **Error**: **shake** horizontal corto en la opción incorrecta (2 oscilaciones, ±6px), luego queda en
  rojo estático.
- **Récord**: el banner se **pega torcido** (−1°), como pegatina puesta con prisa (ver §15, §23.4).
- **Progreso**: transición de `width` `--dur-slow` `--ease-out` al avanzar.
- **Transiciones de página / pestañas**: cross-fade discreto y balancín de pestañas (ver §23.5, §26.3).
- **`prefers-reduced-motion: reduce`**: **sin transforms** (nada de pop/stamp/shake/press/rebote); el
  **color/relleno queda como señal** (el amarillo, el verde/rojo, la posición marcada). El reset base en
  `tokens.css` neutraliza duraciones; los componentes además deben evitar `transform` bajo esa media
  query.
...
**Manifest y theme-color (vigentes):** el manifest (en `vite.config.ts`) usa `theme_color: '#FAF1E2'`
...
- **Un solo lugar audaz**: el **tratamiento de sticker** (contorno de tinta + sombra dura + pulsación) y
  el **amarillo interactivo**. Barra-marcador, trama de puntos, ticks y mono son ecos del mismo álbum,
  tenues para no robarle protagonismo a las banderas.
- **Riesgo del neobrutalismo → novelty/lima ácido**: evitado a conciencia (crema **cálido** + amarillo +
  grotesca chunky, nunca lima ácido ni contornos gratuitos). Si en construcción algo empieza a gritar,
  bajar saturación del amarillo o quitar un contorno antes que añadir efectos.
- **Banderas blancas**: cubiertas por **contorno 2px + sombra dura** en TODA tarjeta de bandera; no es
  opcional.
- **AA y foco**: estados por tinte+borde+texto oscuro; el amarillo siempre lleva tinta encima; foco en
  **tinta gruesa** (`--c-focus`, trazo + offset) visible siempre.
...
> en 360×640**. Todo se mantiene coherente con el sistema vigente (álbum de pegatinas, hoja celeste,
> barra-marcador ámbar, amarillo, tipografías). Aquí solo se **especifica**; implementa
> frontend-engineer.

## 10. El "bottom sheet nota de campo" (feedback modal)

La hoja de feedback es un **objeto físico** del sistema: al responder, **la ficha se desliza hacia
arriba desde el borde inferior**, como una pegatina celeste que se despega y sube sobre la lámina. La
pregunta que acabas de responder **no desaparece**: queda detrás, atenuada por un scrim, para mantener
la continuidad espacial (sigues viendo la bandera/nombre que resolviste).

Es **modal**: mientras está abierta no se puede interactuar con la pregunta (las opciones quedan
inertes). Reutiliza el material de la `FactCard` (superficie **Celeste** + barra-marcador en **Ámbar**),
de modo que la hoja *es* la ficha, ahora a pantalla-ancho y anclada abajo.

### 10.1 Anatomía y medidas

```
   ┌───────────────────────────────┐  ← el resto del viewport: la PREGUNTA
   │  (pregunta atenuada por scrim)│    detrás, atenuada por --scrim (tinta cálida @50%)
   ...
   │▌                              │
   │▌ ┌───────────────────────────┐│  ← §D CTA fijo (amarillo), zona del pulgar
   │▌ │        Siguiente          ││
   │▌ └───────────────────────────┘│
   └───────────────────────────────┘  ← anclado al borde inferior (safe-area)
    ↑ barra-marcador en Ámbar, borde izquierdo, todo el alto (--hoist-width)
```
...
- **Material**: superficie **Celeste** (`--c-note` #CDE9EF), **contorno de tinta** y **sombra dura**
  `box-shadow: var(--shadow-lg)` (la pegatina "flota" sobre la lámina). Es la misma superficie celeste
  de la `FactCard`: la hoja es la ficha.
- **Radios**: solo esquinas **superiores** `--sheet-radius` (`--radius-xl`, 20px); esquinas
  inferiores a 0 (pegadas al borde de pantalla).
- **Barra-marcador (signature)**: barra de `--hoist-width` (**6px**) en **Ámbar** (`--c-accent`) en el
  borde izquierdo, a todo el alto de la hoja. **Se mantiene siempre en ámbar** en ambos estados
  (acierto y error): la barra-marcador es identidad, no semáforo. Por eso el `FactCard` interior **no**
  repite su propia barra — la hoja la aporta una sola vez ("quita un accesorio").
...
- **Padding**: `--sheet-pad` (`--space-5`, 20px) en lados y arriba;
  ...
  inicio (iOS). La barra-marcador (6px) suma a la izquierda; el contenido arranca tras ella.
...
- **§A — Barra de estado** (`aria-live="polite"`, se anuncia al abrir). Fila compacta (~28–32px):
  **sello cuadrado** girado −4° con icono + texto de resultado. Es el **único** sitio con color
  semántico.
  - Acierto: sello `--c-success-tint` + icono `✓` `--c-success`; texto **"¡Correcto!"** en
    `--c-success-ink`, peso 600.
  - Error: sello `--c-error-tint` + icono `✕` `--c-error`; texto **"Era {País}"** en `--c-error-ink`
...
- **§B — Specimen** (identidad de la ficha): fila horizontal con **mini bandera** (`FlagImage`
  `size="sm"`, ~64×43px, con su contorno + sombra) + a la derecha el eyebrow mono/uppercase
  **"Nota de campo"** (`--c-accent-ink`, `--tracking-wide`) sobre el **país en display**
  ...
  bandera también asome atenuada detrás): la ficha siempre lleva su espécimen, refuerza la
...
- **§C — Notas de campo** (`facts`): 1–2 items, `--text-base`, `--leading-normal`, viñeta `·` en
  ámbar (como en `FactCard`). Es la **zona scrollable** si desborda (`overflow-y: auto`;
  `overscroll-behavior: contain` para no arrastrar el fondo). En el caso típico no hay scroll.
- **§D — CTA fijo**: `Button` primario a ancho completo (amarillo `--c-accent-strong`, 52px),
...
Regla anti-semáforo: la superficie celeste + barra-marcador ámbar **no cambian** entre acierto y error;
solo cambian §A (sello/texto) y el icono. Evita que la hoja "parpadee" en verde o rojo pleno y compita
con los colores de la bandera. Restricción coherente con §9.
...
- **Scrim**: capa a pantalla completa (`position: fixed; inset: 0`) con `background: var(--scrim)`
  (**tinta cálida @50%**, `rgba(23,21,15,.50)`; en dark `rgba(0,0,0,.62)`). Atenúa la pregunta pero **la
...
- Opcional (progresivo, sobrio): `backdrop-filter: blur(1.5px) saturate(0.92)` sobre el scrim, para
  desaturar levemente el fondo y que la hoja celeste destaque. Con `@supports`; si no, solo el color.
  No imprescindible.
- **La pregunta detrás no se re-anima ni se mueve**: se queda donde está, solo atenuada. Al cerrar
  la hoja y avanzar, la **siguiente** pregunta hace su entrada `popIn` habitual (§7).
...
- **Entrada**: la hoja `transform: translateY(100%) → translateY(0)` en `--dur-slow` (320ms) con
  **`--ease-pop`** — **sube con un pequeño rebote** (el pop de pegatina del sistema); el scrim
  `opacity: 0 → 1` en `--dur` (200ms).
- **Salida** (al pulsar CTA): la hoja `translateY(0) → 100%` en `--dur` (200ms, más rápida que la
  ...
  siguiente pregunta (que hace su `popIn`).
- **`prefers-reduced-motion: reduce`**: **sin translate ni rebote**. La hoja y el scrim solo hacen
  **fade** (`opacity 0 → 1` en `--dur`) al entrar y fade-out al salir. Nada de deslizamiento. (El reset
  de `tokens.css` ya neutraliza duraciones; el componente además debe evitar `transform` bajo esta
  media query.)
- La barra de estado §A puede conservar el **stamp** del acierto (el sello ✓ que cae girado −4° y
  asienta) y el *shake* corto del error en el sello ✕ — pero **solo el sello/chip**, no toda la hoja, y
  suprimidos bajo reduced-motion.
...
- **name→flag** (grid 2×2 — debe caber): barra 44 + pad 24 + prompt 20 + gap 12 +
  **nombre de país** (bajar a `--text-xl` 22px en juego, 1–2 líneas ≈ 30–56px) + gap 12 + **grid**.
  ...
  tarjeta `size="sm"`, contorno + sombra); no se toca la escala definida en §5.1/§5.2.
...
- **Tokens añadidos a `tokens.css`** (§ Iteración 2):
  ...
  - `--scrim: rgba(23,21,15,.50)` (tinta cálida @50%; dark `rgba(0,0,0,.62)`).
...
- **Componentes afectados (no los toco; para frontend-engineer)**:
  ...
  - `AnswerFeedback` se reencarna como contenido de la hoja: barra de estado §A + `FactCard`
    (sin su barra-marcador propia, que ahora la pone la hoja) + specimen §B + CTA §D. `FactCard` gana
    la opción de **no** pintar su `::before` de barra-marcador cuando vive dentro de la hoja.
  - La `FlagImage` héroe usa `--flag-hero-max-h` (contenedor con `aspect-ratio` + `max-height`).
- Esta iteración fue puramente de **layout + un patrón modal**, dentro del sistema visual vigente
  (álbum de pegatinas): la hoja es una **superficie celeste** con **barra-marcador ámbar**, y solo
  cambió el valor de `--scrim`. Sin cambios de paleta, tipografía ni signature.
...
> frontend-engineer. Todo se mantiene dentro del sistema visual vigente (álbum de pegatinas,
> barra-marcador, amarillo con disciplina, mono para lecturas de datos). **No se toca la paleta
...
> visual nuevo — reutiliza el signature (el tick que "prende" a ámbar) para la selección y el récord,
...
- **Regla de progreso** (§5.6): *posición en la ronda*. **Crece** izq→der, **ámbar**, con ticks,
  inline en la fila.
- **Mecha del countdown**: *tiempo de la pregunta*. **Se consume** (der→izq), en **tinta media** y
...
- **Posición**: hija del `.topbar` (que pasa a `position: relative`), a sangre en el borde inferior
  (`left/right/bottom: 0`), alto `--countdown-track-h` (**4px**). Es **absolute** → **cero altura
...
- **Color (urgencia creciente, sin ansiedad constante):**
  - Lane (fondo): `--c-surface-2` (superficie hundida; `--c-border` ya no sirve de relleno, es tinta).
  - Relleno **calmo** (t > 3 s): `--c-ink-2` (tinta media; instrumento, no alarma). AA como
    elemento de UI.
...
- **Sin pulso por defecto** ("quita un accesorio"): el cambio de color basta. Un latido opcional del
  relleno rojo se **descarta** aquí por sobriedad y por reduced-motion (§23.3 lo reintroduce a pedido
  del roadmap B).
...
.countdownFuse {                              /* lane a ras del borde inferior */
  ...
  height: var(--countdown-track-h);           /* 4px */
...
- Competitivo = **single-select** de **filas-dossier** (una decisión seria), agrupadas, con **lectura
  de récord** en ámbar. Filas grandes y escaneables, no pills.
...
│ │▌ Mundo             194 países  │ │     barra-marcador SIEMPRE ámbar
...
│ │                Récord 3.200 pts│ │  récord en ámbar (--c-accent-ink)
│ └───────────────────────────────┘ │
...
- **Tick de barra-marcador** a la izquierda (`--hoist-width`, radio del lado izquierdo): **neutro**
  (`--c-border-strong`, tinta) sin seleccionar, **ámbar** (`--c-accent`) al seleccionar. El tick **es**
  el indicador de radio: "esta es la carta que has sacado". Uso on-brand del signature para el
...
- **Récord** a la derecha, mono `--text-xs`:
  - Con récord: `"Récord {pts} pts"` en `--c-accent-ink` (ámbar, la única cifra premiada — coherente
    con `.streak` de ResultPage). `{pts}` con formato `toLocaleString('es-ES')` → "3.200".
  - Sin récord: `"Aún sin récord"` en `--c-ink-3`.
- **Estados**: normal (superficie + borde de tinta + `--shadow-sm`); **seleccionada**
  (`--c-accent-tint` + borde `--c-accent` + tick ámbar); **foco** (tinta gruesa / `--shadow-focus`).
  Alto ≥ 64px (tap cómodo).

**Mundo** (destacada, primera, sobre los dos grupos): misma anatomía con más presencia — tick
**siempre ámbar**, radio `--radius-xl`, y un descriptor en la meta ("Los 194 países · 20 preguntas").
...
.region::before {                              /* tick de barra-marcador (signature) */
...
.region:has(:checked)::before { background: var(--c-accent); }        /* tick prende a ámbar */
...
panel **"prende" a ámbar** (eco literal del acierto, §7) y se **pega torcido** como pegatina de logro.

**Estado A — ¡Nuevo récord!** (puntaje actual > mejor previo, o primer récord):
```
┌▌────────────────────────────┐   ▌ barra-marcador de logro (ámbar), izquierda
│ ¡Nuevo récord!               │   eyebrow mono uppercase, --c-accent-ink
│ 3.480                        │   cifra en ÁMBAR (--c-accent)
│ Mejor racha: 9 seguidas      │   (línea existente)
│ Superaste tu récord de 3.200 │   mono --c-ink-2 (qué batiste)
└──────────────────────────────┘
```
- El panel gana `border-color: --c-accent`, una **barra-marcador** ámbar a la izquierda
  (`--hoist-width`, radio `--radius-md`), la cifra `.scorePoints` en `--c-accent`, y **se estampa
  torcido** al montar (`recordStamp`, `--ease-pop`, reposo a **−1°**) — la pegatina de logro puesta
  con prisa. Bajo reduced-motion, sin stamp ni tilt (solo el color/barra-marcador).
...
con la marca a batir, mono `--text-sm`, `--c-ink-2`: **"Récord {mejor} pts"**. Sin ámbar de logro
(el ámbar se reserva para el logro real).
...
.scorePanel.isRecord {
  ...
  padding-left: calc(var(--space-4) + var(--hoist-width));
  transform: rotate(-1deg);                     /* pegatina de logro puesta con prisa */
  animation: recordStamp var(--dur) var(--ease-pop);
}
.scorePanel.isRecord::before {
  content: ''; position: absolute; inset: 0 auto 0 0;
  width: var(--hoist-width); background: var(--c-accent);   /* barra-marcador ámbar */
...
@keyframes recordStamp {
  0%   { transform: rotate(-1deg) scale(0.9); }
  60%  { transform: rotate(-1deg) scale(1.03); }
  100% { transform: rotate(-1deg) scale(1); }
}
@media (prefers-reduced-motion: reduce) { .scorePanel.isRecord { animation: none; transform: none; } }
```
...
- **Chips sin cambios**: mismo `.chip`/`.selected` de `ContinentPicker.module.css` (cero CSS de chip
  nuevo; misma a11y `aria-pressed`). El chip elegido usa la pegatina amarilla (`--c-marker`).
...
- **Icono**: reloj `◷` en el sello `--c-error-tint` / `--c-error` (misma caja del icono §A). El glifo
...
- `--countdown-track-h: 4px;` — grosor de la **mecha** del countdown (§13.1). Del mismo trazo fino del
  signature (junto a `--hoist-width: 6px`). **Único token de diseño nuevo de esta iteración.**
...
`--c-accent-ink` / `--c-accent-tint` / `--c-surface-2` / `--c-border-strong`; `--hoist-width`; radios;
`--shadow-sm` / `--shadow-focus`; `--ease-pop` / `--ease-spring` / `--dur`; escala tipográfica y
`--font-mono` / `--font-display`. **Dark mode**: gratis (todos esos tokens ya tienen variante dark).
**Sin tokens de color nuevos.**
...
- **Masthead compacto** (reemplaza al hero de la Home): **trama de puntos** de fondo, eyebrow "Guía de
  campo · {N} países", h1 "Banderas del mundo" y el **espécimen del día en miniatura** (FlagImage `sm`
...
- **Pestañas de carpeta** (signature de la iteración): dos enlaces ("Casual" / "Competitivo") con
  forma de pestaña de expediente — radios solo arriba, la activa **abre hacia el panel**. La versión
  vigente las convierte en un **balancín de pegatina** (la activa despega en amarillo y la inactiva se
  hunde; ver §26.3, que retiró la barra estática original de esta iteración). Son `NavLink`s
  (aria-current automático), no botones: la pestaña ES la ruta.
...
- **El tick marcador es el indicador de radio**: `--hoist-width`×18px inset vertical centrado (nunca
  toca esquinas), neutro (tinta) en reposo; al elegir pasa a ámbar, crece a 26px y la fila se tinta
...
- Récord por fila: **solo la cifra** (`toLocaleString('es-ES')`) en ámbar; vacío = "—" (ink-3) con
...
Sin tokens nuevos: pestañas y ledger reutilizan el signature, radios, capilares y la escala mono
existente.
...
> **sensación de juego** sin romper la sobriedad del sistema. La consigna del roadmap manda:
> **"el destello del récord, no confeti"**. Aquí se **especifica** la dirección visual/motion/sonora con
...
1. **El sonido es de instrumento, no de arcade.** El sistema tiene un instrumento **de metal cálido**:
   los sonidos son **metal cálido templado** (campanilla / diapasón) y **madera** (el toc del
   cuaderno), nunca ondas cuadradas chillonas. **Firma sonora: todo está afinado a La (A440)** —
   el diapasón del instrumento. Los positivos giran sobre la tríada de La mayor (La–Do♯–Mi);
...
- **Metal cálido** (positivos, tic): síntesis aditiva de parciales `f, 2f, 3f` con amplitudes `1.0,
  0.5, 0.18` (octava rica + tercer parcial que da el filo metálico). Inarmonicidad leve para brillo:
...
|---|--------|-----------------|------------|-------------------------------|----------|--------------------|
| 1 | **Acierto** | Metal `f,2f,3f` | **Do♯5 = 554.37** (una nota) | ataque 3 ms · τ 55 ms | **150 ms** | 1.00 · **−3 dBFS** |
| 2 | **Hito de racha** | Metal (hermano del acierto) | **Do♯5 554.37 → Sol♯5 830.61** (quinta ↑; 2ª nota entra a 55 ms, solapa) | ataque 2 ms · τ 40 ms por nota | **130 ms** | 0.85 · **−4 dBFS** |
...
| 5 | **Tic de urgencia** | Metal corto `f,2f` (2f a 0.20) | **La5 = 880** | ataque 1 ms · τ 18 ms | **55 ms** | 0.45 · **−8 dBFS** |
| 6 | **Récord nuevo** | Metal, arpegio + cola | **La4 440 · Do♯5 554.37 · Mi5 659.25 · La5 880** (onsets ~70 ms) | notas 1–3: ataque 4 ms · τ 70 ms; nota 4: ataque 8 ms · τ 180 ms + trémolo 5 Hz prof. 12% y parcial 4f a 0.10 (el "sheen") | **520 ms** | 0.90 · **−2 dBFS** |

**Carácter buscado por evento:**
- **Acierto** — un "ting" de metal limpio y cálido; suena en *cada* acierto, así que ligero y corto.
...
- **Tic de urgencia** — un escape de metal muy corto y **muy bajo** (−8 dBFS): se repite en t=3,2,1
...
- **Récord** — el único "fanfarria": la tríada de La mayor **ascendiendo** y la última nota que
  **florece** con trémolo (eco sonoro del destello de rotulador visual, §23.4). Cálido y breve, jamás
  jingle de 8-bit.
...
- `title` para puntero: **"Silenciar sonido"** cuando está ON, **"Activar sonido"** cuando está
  silenciado.
- Toque mínimo 44px, foco visible (tinta gruesa heredada de `:focus-visible`).
...
countdown): `×1.3` en `--font-mono`, `--text-sm`, `tabular-nums`, color `--c-accent-ink` (ámbar). Se
...
- **Motion**: al subir el multiplicador, `streakPop` con el **`--ease-spring` existente** (el mismo
  ...
  dígito se actualiza en el pico. Sin brillo extra (el ámbar del color ya lo dice).
- **Reduced-motion**: **sin scale**. La cifra simplemente **cambia** de valor (p. ej. `×1.2`→`×1.3`) en
  ámbar — el cambio de número + el ámbar **son** la señal. Dignísimo y AA.
...
Suma al *stamp* de escala que ya existe (§7) un **anillo verde que florece y se apaga** en el instante
...
- **Opciones (flag→name / name→flag)**: sobre la opción `.correct`, `correctFlash` una vez, `--dur`
  ...
  stamp (escala con leve rotación) ya definido en §7. Es un pulso de luz hacia afuera, no un parpadeo
  de fondo.
- **Escrito (`type-name`)**: al acertar, el borde del input pasa a Verde sello (§5.3) **y** el mismo
...
- **Reduced-motion**: sin anillo ni escala. El estado verde **estático** (borde 2px `--c-success` + ✓,
...
- Cuando `.countdownFuse.isWarn`, el relleno `> i` late en **opacidad** `1 → 0.5 → 1` a ~1 Hz
  ...
  **sincronizado** con el tic sonoro y con el dígito rojo. Solo el relleno late (4px); nada de glow que
...
### 23.4 Barrido de destello de rotulador sobre el banner «¡Nuevo récord!»
El único gesto "fanfarria" visual. Sobre el `.scorePanel.isRecord` (§15, que ya *prende* a ámbar: borde
+ barra-marcador + cifra + stamp), un **destello de rotulador** cruza el panel **una sola vez**, como
luz que resbala sobre un marcador brillante. **No confeti.**

- Mecanismo: pseudo-elemento `::after` con el gradiente **`--brass-sweep`** (§25: banda ámbar con
  núcleo casi-blanco, definida en tokens) que **traslada** de izquierda a derecha y sale de cuadro.
- Timing: `brassSweep var(--dur-sheen) var(--ease-standard) 160ms 1 both` — arranca **tras** el stamp
  ...
  esquinas (`--radius-md`); la barra-marcador va por dentro, no le afecta.
- **Reduced-motion**: **sin barrido**. La celebración queda en el estado estático del §15 (borde/
  barra-marcador/cifra en ámbar) — que ya es "de marca". El **sonido de récord sí suena** (audio ≠
  motion, §20.3).
...
Mejora progresiva: un **cross-fade corto** del panel (nada de deslizamientos largos). react-router 6.30
no integra la View Transitions API, así que se envuelve la navegación en `document.startViewTransition`.

- **Qué transiciona**: el **panel** (el contenido bajo las pestañas) hace un **cross-fade** — se nombra
  su contenedor `view-transition-name: play-panel`. Las **pestañas** ya no quedan mudas en `root`: cada
  una lleva su propio `view-transition-name` (`play-tab-a` / `play-tab-b`) para que su **balancín**
  (sube/baja) sea visible durante la navegación (§26.3); el masthead queda en `root`. Así se siente un
  "cambio de carpeta" con las solapas basculando y la hoja detrás cambiando. También aplica al salto
  `/` ⇄ `/competitivo` ⇄ `/jugar` (arranque y salida de partida): mismo cross-fade de `--dur-vt`.
...
`<span>` cuyo `background` es `currentColor`; así se tiñe con `color` (tinta o ámbar) usando el mismo
...
- **Colocación**: dentro de la celda `.zone`, **antes** del label (tras el tick marcador). `.zone` pasa
...
- **Tinta**: reposo `--c-ink-3` (la "tinta al 60-70%" del roadmap); **fila seleccionada** `--c-accent`
  (ámbar) — hereda el lenguaje del tick que "prende". `Mundo` (insignia): silueta en `--c-ink-2` en
  reposo (un punto más presente), ámbar al seleccionar.
...
- **Ficha del país — sí.** Al abrir `/explorar/{code}` se carga **una** silueta (respeta el presupuesto:
  ...
  `--c-ink-2`. Lee como "contorno del espécimen + su nombre", sin competir con la bandera `lg` (que
  aporta el color). Alineada al centro óptico del bloque nombre/oficial.
...
|-------|---------------|-----|
| `--c-brass-glint` | `rgba(255,255,255,0.85)` | núcleo casi-blanco del destello de récord (§23.4) |
| `--c-brass-sheen` | `rgba(245,158,11,0.45)` | banda ámbar del destello (§23.4) |
| `--brass-sweep` | `linear-gradient(105deg, transparent 34%, sheen 46%, glint 50%, sheen 54%, transparent 66%)` | gradiente del barrido (usa los dos anteriores) |
| `--glow-correct` | `rgba(30,158,82,0.50)` | anillo de confirmación del acierto (§23.2) |
| `--dur-pulse` | `1000ms` | latido de la mecha en warn, en fase con el tic (§23.3) |
| `--dur-sheen` | `900ms` | barrido del récord (§23.4) |
...
Dark: `--c-brass-glint` (`rgba(255,245,200,0.65)`) / `--c-brass-sheen` (`rgba(251,191,36,0.35)`) /
`--glow-correct` (`rgba(87,192,106,0.45)`) tienen override (ámbar/amarillo y verde claros del dark). El
resto es neutro al tema. **El sonido no añade tokens CSS** (WAVs + `sound.ts`). Todo lo demás reutiliza
tokens existentes (`--c-accent`, `--c-accent-ink`, `--c-ink-2/3`, `--c-error`, `--ease-spring`,
`--dur`, escala mono).
...
- Componentes de opción (`FlagToNameQuestion`/`NameToFlagQuestion`) y `TypeNameQuestion` — `correctFlash`
  (§23.2).
- `ResultPage` / `.module.css` — barrido del destello `brassSweep` + `record` sonoro (§23.4, §22.2).
...
> sin romper la sobriedad del sistema (**el destello del récord, no confeti**, la regla no cambia).
> Propiedad de ui-designer: este doc + `tokens.css`. Implementa frontend-engineer con esta spec; **cero**
> cambios de paleta o tipografía, un solo bloque de tokens nuevos (§26.7, "Iteración C"). Numeración
> continúa tras §25.

## 26. Módulo Jugar: dirección de la iteración

Tres cambios duros, un solo hilo conductor: **el papel se vuelve mecanismo de pegatinas.** Donde B dejó
controles que sólo cambiaban de color, C introduce la **física de la pegatina**: piezas que se deslizan,
bascullan y prenden. El mismo lenguaje une las tres piezas: el `SegmentedControl` gana un **thumb amarillo
que se desliza** (§26.2), las pestañas de carpeta se vuelven un **balancín** — la activa despega y la
inactiva se hunde (§26.3), y el casual abandona el formulario plano para convertirse en un **índice de
especímenes** — un ledger multi-select con las siluetas de zona a 20px, hermano del libro de registro
competitivo (§26.4). La consigna de Chanel se respeta: **todo el movimiento nuevo es la misma física de
pegatina en ámbar/amarillo**, ningún color ni efecto extra; el "brillo" fuerte sigue reservado al récord
(§23.4). El casual pasa de "3 campos apilados sin alma" a "rellenar una hoja de expedición".

**Por qué NO es un default de IA:** ningún slider material-design con thumb blanco elevado ni pastillas
de colores; el thumb es una **pegatina amarilla con sombra dura** sobre papel, medida por índice con
`calc` (no una librería), y el selector de categorías es un **ledger de especímenes con siluetas de
contorno** — un artefacto del tema (álbum de pegatinas), no un multiselect genérico de chips.

### 26.1 Principios de C (léelos antes de tocar nada)
1. **Un solo lenguaje nuevo: la física de pegatina.** El thumb amarillo que se desliza, el balancín de
   las pestañas y el "prender" del tick son variaciones de la **misma pegatina** en ámbar/amarillo. No se
   inventa un segundo lenguaje de motion.
...
4. **Salida digna en `prefers-reduced-motion` para CADA animación** (§26.6). Sin excepción: el reset de
   ...
   (el color/tinte/ámbar/amarillo porta la señal, no el movimiento).
5. **Móvil 360–414 primero, AA, foco visible, targets 44px.** El thumb va *detrás* del texto; el foco
   sigue siendo el `outline` de tinta del botón/casilla.
...
`background`. C sustituye ese background por **una sola pieza `.thumb`** — una **pegatina amarilla**
(`--c-marker`) con sombra dura — que se **desliza** hasta la opción elegida. Un único componente
compartido cubre `direction="row"` (Preguntas casual, Mixto|Escrito competitivo) y `direction="column"`
(Modo casual).
...
/* El carril del slider: una pegatina amarilla, tamaño de UN segmento. */
.thumb {
  position: absolute;
...
.selected { color: var(--c-on-marker); font-weight: var(--weight-semibold); }  /* tinta sobre amarillo */
```
...
pestaña activa está **DESPEGADA** (sube 3px, pegatina amarilla `--c-marker` con sombra dura) y la inactiva
**PEGADA** (baja 3px, superficie hundida `--c-surface-2`, sin sombra). Al cambiar de pestaña, **ambas
morfean a la vez en sentidos opuestos** — sube una, baja la otra. Mismo lenguaje de pegatina que el thumb
del §26.2 y el tick del §26.5 → coherencia total.

**Técnica — cada pestaña con su propio `view-transition-name` + fallback CSS:**
- Cada pestaña es un `NavLink` con `aria-current`; se le asigna un `view-transition-name` **estable por
  posición** (primera = `play-tab-a`, segunda = `play-tab-b`), **independiente** de cuál esté activa. Así
  el navegador puede emparejar la misma pestaña old→new y morfear su caja.
- **Dos caminos de animación que no chocan** (uno por navegador):
  - **Con View Transitions API** (Chromium): el VT que ya envuelve la navegación de pestañas (§23.5)
    captura **cada pestaña por separado** y **morfea** su caja old→new = el **sube/baja** del balancín.
    **Lección clave**: si la pestaña no tiene nombre propio cae en el snapshot `root` y su desplazamiento
    queda **enmascarado** (solo se ve un fundido); con `play-tab-a`/`play-tab-b` el balancín **se ve**.
  - **Sin VT** (Firefox / Safari viejo): `navigate` normal, **la instancia de `PlayPage` persiste**
    ...
    reconcilia y reutiliza; sólo cambia `aria-current`), así que la **transición CSS de `transform`**
    anima el balancín. ⚠️ **No añadir `key={tab}` a `PlayPage`**: forzaría remount y mataría este fallback.

```css
/* PlayPage.module.css */
.tabs {
  position: relative;                /* ancla del grupo */
  display: grid;
  grid-template-columns: 1fr 1fr;
  gap: var(--space-2);
}
.tab {                               /* NavLink de pestaña */
  display: flex; align-items: center; justify-content: center;
  min-height: var(--tap-min);
  border: var(--border-mid) solid var(--c-border);
  border-radius: var(--radius-md) var(--radius-md) 0 0;   /* radios solo arriba */
  background: var(--c-surface-2);     /* PEGADA por defecto: hundida, sin sombra */
  color: var(--c-ink-2);
  transform: translateY(3px);
  transition: transform var(--dur-slide) var(--ease-out),
              background-color var(--dur-fast) var(--ease-standard);
}
.tab:first-child { view-transition-name: play-tab-a; }   /* nombre estable por posición */
.tab:last-child  { view-transition-name: play-tab-b; }
.tab[aria-current='page'] {          /* DESPEGADA: sube, amarilla, sombra dura */
  background: var(--c-marker); color: var(--c-on-marker);
  transform: translateY(-3px);
  box-shadow: var(--shadow-sm);
}
/* Duración del morph del balancín en navegadores con VT (independiente del panel). */
:global(::view-transition-group(play-tab-a)),
:global(::view-transition-group(play-tab-b)) {
  animation-duration: var(--dur-slide);
  animation-timing-function: var(--ease-out);
}
```

**Se elimina** `.tabHoist` (la barra deslizante de la versión previa de C) y `.tabActive::before` (el
hoist estático de §19.2). El **panel** sigue con su cross-fade `play-panel` de `--dur-vt` (§23.5),
independiente del balancín de las solapas; los nombres de las pestañas ("Casual"/"Competitivo") viajan
ahora en sus propios grupos de View Transition, no en `root`.

**`prefers-reduced-motion`:** el guard global de §23.5 (`::view-transition-group(*) { animation: none }`)
anula el morph; el reset global anula la transición de `transform`. Las pestañas **saltan** a su estado
(la activa arriba y amarilla, la inactiva hundida). La posición + el color siguen marcando la pestaña.
Digno.
...
- **Tick marcador = indicador de casilla**: `::before` idéntico al competitivo (`--hoist-width`×18px,
  inset, neutro/tinta en reposo). Al marcar: ámbar + crece a 26px + la fila se tinta `--c-accent-tint` —
  pero con **micro-tacto de "prender"** (§26.5). Varias filas pueden estar encendidas a la vez
  (multi-select).
- **Silueta de zona** (§24.1, misma técnica `mask` + `--shape-src`): `--shape-md` (**20px**), reposo
  `--c-ink-3`; fila marcada → `--c-accent` (ámbar, hereda el "prende" del tick). Assets ya precacheados
...
- **"Todos"** = primera fila del card, tratamiento insignia (como el `Mundo` competitivo): silueta
  `mundo.svg` en `--c-ink-2` en reposo, ámbar cuando `isAll`. Su casilla refleja `isAll`
...
- **El tick** pasa de neutro (tinta) a ámbar y **crece 18→26px con `--ease-spring`** (§26.4.2): un pequeño
  "snap" de encendido, no un fundido. El spring aquí es seguro (una barra de 6px que crece; no hay carril
  que desbordar).
- **La silueta** cruza de `--c-ink-3` a `--c-accent` con `--ease-out` (transición de `color`): el
  contorno del espécimen "se enciende" en ámbar a la vez.
...
- **`prefers-reduced-motion`:** el reset global pone las transiciones a 0 → el estado marcado (tinte +
  tick ámbar 26px + silueta ámbar) aparece **instantáneo**. El ámbar **es** la señal.
...
|---|-----------|-------|-----------|-------------------|------------------------|
| C1 | **Thumb slider** (pegatina amarilla) | `SegmentedControl` (fila y col.) | cambia `value` → `--seg-index` | `transform` · `--dur-slide` `--ease-out` | thumb **salta** al segmento (sigue mostrando selección) |
| C2 | **Pestañas balancín** | `PlayPage .tab` | cambia ruta `/`⇄`/competitivo` → `aria-current` | VT morph `play-tab-a`/`play-tab-b` (transform sube/baja) **o** `transform` · `--dur-slide` `--ease-out` | guard §23.5 + reset → **saltan** a su estado |
| C3 | **Tick "prende"** | fila del ledger casual | `:has(:checked)` | `height`+`background` · `--dur-fast` `--ease-spring` | estado marcado **instantáneo** (ámbar 26px) |
| C4 | **Silueta se enciende** | `.zoneShape` casual | `:has(:checked)` | `color` · `--dur-fast` `--ease-out` | color ámbar **instantáneo** |
...
a 0). Sin layout shift: el thumb es absoluto, las pestañas basculan sobre su propio sitio, y el índice de
especímenes reserva su hueco.

## 26.7 Tokens nuevos (bloque "Iteración C" en `tokens.css`)
| Token | Valor (light) | Dark | Uso |
|-------|---------------|------|-----|
| `--dur-slide` | `220ms` | (neutro) | deslizamiento del thumb (§26.2) y del balancín de pestañas (§26.3) |
| `--shadow-thumb` | `2px 2px 0 var(--c-shadow)` | `2px 2px 0 #000` (vía `--c-shadow`) | **sombra dura** que eleva el thumb del carril como pieza física (§26.2) |

Todo lo demás reutiliza tokens existentes: `--c-marker`/`--c-on-marker`, `--c-accent`/`--c-accent-tint`/
`--c-accent-ink`/`--c-ink-2/3`, `--shape-md` (20px), `--ease-out`/`--ease-standard`/`--ease-spring`/
`--ease-pop`, `--dur-fast`/`--dur-slow`, `--radius-*`, `--hoist-width`, `--space-*`. **Cero** cambios de
paleta o tipografía.
...
- **`SegmentedControl.module.css`** — `.group` a `position: relative; isolation: isolate`; añadir `.thumb`
  (fila y `.column .thumb`) con los `calc` de tamaño/desplazamiento, `background: var(--c-marker)` y
  `box-shadow: var(--shadow-thumb)`; quitar el `background` de `.selected` (lo aporta el thumb), dejar
  `color: var(--c-on-marker)` + peso; `.segment` a `position: relative; z-index: 1; flex: 1 1 0`. (§26.2)
- **`PlayPage.tsx`** — las dos pestañas son `NavLink` con `aria-current`. **No** poner `key={tab}` en
  `PlayPage` (rompería el fallback CSS del balancín). Eliminado el `<span class="tabHoist">`. (§26.3)
- **`PlayPage.module.css`** — `.tabs` a `position: relative`; `.tab` con radios solo arriba, **PEGADA**
  por defecto (`--c-surface-2`, `translateY(3px)`, sin sombra) y **DESPEGADA** en `[aria-current='page']`
  (`--c-marker` + `--c-on-marker`, `translateY(-3px)`, `--shadow-sm`); `view-transition-name`
  `play-tab-a`/`play-tab-b` por posición (`first`/`last`) + `::view-transition-group(play-tab-a/b)` con
  `--dur-slide`; **eliminar** `.tabHoist` y `.tabActive::before`. (§26.3)
...
- **`tokens.css`** — bloque "Iteración C": `--dur-slide`, `--shadow-thumb` (+ override dark vía `--c-shadow`).
  (§26.7)
...
Tarjeta de superficie normal con **barra-marcador ámbar** en el canto izquierdo (6px, inset 12px de
...
`--c-ink-2`, CTA compacto **amarillo** (`--c-accent-strong`, 44px, NO el Button de 52px: es un
